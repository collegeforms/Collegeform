...
// Import clients data from your JSON file

const Logos = () => {
  const [loadSwiper, setLoadSwiper] = useState(true);
...
console.log(logos);
  ...
  useEffect(() => {
    ...
    const fetchLogos = async () => {
      try {
        const response = await axios.get('https://www.collegeforms.in/api/logos');
  ...
  return (
    ...
    <div class="section-heading pt-3">
      ...
      <div className="clients-slider5" dir="ltr">
          <Swiper
            ...
            autoplay={{
              delay: 0, // Set autoplay delay to 0 for continuous movement
              disableOnInteraction: false, // Keep autoplay even after interaction
            }}
            loop={true}
            allowTouchMove={true} // Allow touch/swiping
          ...
          >
            {logos.map((logos, i) => (
              <SwiperSlide key={i}>
                  <img src={`https://www.collegeforms.in${logos.image}`} alt={`Client ${i + 1}`} />
              </SwiperSlide>
            ))}
          </Swiper>
      </div>
      ...
      <div className="clients-slider5" dir="rtl">
          ...
          >
            {logos.map((logos, i) => (
              <SwiperSlide key={i}>
                  <img src={`https://www.collegeforms.in${logos.image}`} className='w-100' style={{width:"300px"}} height={"300px"} alt={`Client ${i + 7}`} />
              </SwiperSlide>
            ))}
          </Swiper>
      </div>
