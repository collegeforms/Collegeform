...
const AdminSpecialization = () => {
  ...
  const fetchSpecializations = async () => {
    try {
      const response = await axios.get('http://localhost:5000/specializations');
  ...
  const fetchPriceRanges = async () => {
    try {
      const response = await axios.get('http://localhost:5000/priceRanges');
  ...
  const handleSpecializationSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:5000/specializations', { name: specialization });
  ...
  const handlePriceRangeSubmit = async (e) => {
    ...
    if (min < 100000 || max > 2000000 || min >= max) {
    ...
    try {
      const response = await axios.post('http://localhost:5000/priceRanges', priceRange);
      if (response.status === 201) {
  ...
  const handleDeleteSpecialization = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/specializations/${id}`);
  ...
  const handleDeletePriceRange = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/priceRanges/${id}`);
