...
const Colleges = () => {
  ...
  const fetchColleges = async () => {
    try {
      const response = await axios.get("https://www.collegeforms.in/api/colleges");
  ...
  const handleAddOrUpdateCollege = async (collegeData) => {
    try {
      ...
      if (editCollege) {
        await axios.put(`https://www.collegeforms.in/api/colleges/${editCollege._id}`, formData, {
  ...
  const handleDeleteCollege = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`https://www.collegeforms.in/api/colleges/${id}`);
            ...
            {colleges.map((college) => (
              <TableRow key={college._id} sx={{ "&:hover": { backgroundColor: "#E8F5E9" } }}>
                ...
                <TableCell align="center">
                  <img src={`https://www.collegeforms.in${college.image}`} alt={college.name} style={{ width: "50px" }} />
                </TableCell>
