...
const AddLogo = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post("https://www.collegeforms.in/api/logos", formData, {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://www.collegeforms.in/api/logos/${id}`);
          ...
          <Grid item xs={12} sm={6} md={3} key={logo._id}>
            <Card sx={{ position: "relative", boxShadow: 3 }}>
              <CardMedia
                ...
                image={`https://www.collegeforms.in${logo.image}`} // Adjust API path
                alt="Logo"
              />
