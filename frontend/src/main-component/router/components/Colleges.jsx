...
const Colleges = () => {
    ...
    // Function to clean and format the course data
    const cleanCourseData = (courses) => {
        return courses.map((course) => {
            return course.replace(/[^\w\s]/gi, "").trim();
        });
    };
    ...
    const loadMoreColleges = () => {
        setVisibleColleges((prevVisibleColleges) => prevVisibleColleges + 5);
    };
    ...
    return (
        <div style={{ backgroundColor: "#F4F4F7" }}>
            <div className="container pt-5">
                    ...
                    {colleges.length > 0 ? (
                        colleges.slice(0, visibleColleges).map((college) => (
                            <div key={college._id} className="col-md-6 mb-4">
                                <div className="college-card bg-white border border-2 shadow-sm">
                                    <div className="row">
                                        <div className="col-md-4 clg-card">
                                            <img
                                        ...
                                        <div className="col-md-8">
                                            <div className="college-info d-flex justify-content-between">
                                                <div>
                                                    <h5>{college.name}</h5>
                                                    ...
                                                    <p className="rating">
                                                        {renderStars(college.reviews)}
                                                        <span className="ms-2">({college.reviews} Reviews)</span>
                                                    </p>
                                                </div>
                                            ...
                                            <div className="details">
                                                <div className="pill-container">
                                                    {cleanCourseData(college.courses).map((course, index) => (
                                                        <span key={index} style={{ fontSize: "10px" }} className="pill-button">
                                                            {course}
                                                        </span>
                                                    ))}
                                                </div>
                                                <button className="btn apply-btn">Apply Now</button>
                                                <button className="btn brochure-btn ms-3">More Info</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                {/* Load More Button */}
                {colleges.length > visibleColleges && (
                    <div className="row justify-content-center">
                        <div className="col-md-3 mb-5">
                            <button onClick={loadMoreColleges} className="btn btn-primary">
                                Load More
                            </button>
                        </div>
                    </div>
