...
const EditCollege = () => {
  ...
  const fetchCollegeData = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/colleges/id/${id}`);
      const collegeData = response.data.college || response.data;
      ...
      setSnackbar({ open: true, message: "Error fetching college data", severity: "error" });
    }
  };
  ...
  const handleAddNewExam = async () => {
    ...
    try {
      ...
      setCollege({ 
        ...college, 
        exams: [...college.exams, response.data.name] 
      });
      setNewExam("");
  ...
  const handleDeleteExam = async (examId, examName) => {
    try {
      ...
      if (college.exams.includes(examName)) {
        setCollege({
          ...college,
          exams: college.exams.filter(exam => exam !== examName)
        });
      }
  ...
  const handleNameChange = (e) => {
    ...
    setCollege(prev => ({
      ...prev,
      name: newName,
      // Auto-generate slug when name changes, but allow manual override
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("slug", college.slug || generateSlug(college.name));
      ...
      const response = await axios.put(`${API_URL}/api/colleges/${id}`, formData, {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ maxWidth: '100%' }} encType="multipart/form-data">
        ...
        <Accordion defaultExpanded>
          ...
          <AccordionDetails>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6}>
                <SearchableSelect
                  ...
                  selectedValues={college.collegeType}
            ...
            <TextField
              ...
              helperText={`${college.shortDescription.length}/200 characters`}
            />
                ...
                <Box sx={{ mt: 1, maxHeight: 150, overflow: 'auto' }}>
                  {availableExams.map((exam) => (
                    <Chip
                      ...
                      size="small"
                      sx={{ m: 0.5 }}
                      color={college.exams.includes(exam.name) ? "primary" : "default"}
                    />
                  ))}
                </Box>
              </Grid>
                ...
                <Box sx={{ mt: 2 }}>
                  ...
                  <label htmlFor="upload-image">
                    <Button 
                      ...
                      size="small"
                    >
                      {imageFile ? "Change Main Image" : "Upload New Main Image"}
                    </Button>
                  </label>
