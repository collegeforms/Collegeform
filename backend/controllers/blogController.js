  ...
  try {
    ...
    // Create new draft
      ...
      const newDraft = new Blog({
        ...
        status: 'draft',
        slug: null, // Explicitly set to null for drafts
        faqs: faqs
      });
    ...
    if (error.code === 11000 && error.keyPattern.slug) {
      return res.status(400).json({
        success: false,
        message: "Duplicate slug error. Please try again with a different title.",
...
export const publishBlog = async (req, res) => {
  try {
    ...
    if (error.code === 11000 && error.keyPattern.slug) {
      // Retry with a different timestamp if slug conflict occurs
      try {
        const blog = await Blog.findById(req.params.id);
        const baseSlug = blog.title
          .toLowerCase()
