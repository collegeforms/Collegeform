...
router.get("/sitemap.xml", async (req, res) => {
    try {
        console.log("🔄 Generating valid XML sitemap...");
        ...
        // ✅ FIXED: Fetch colleges with proper filtering
        try {
            // Only get colleges that have slugs and are active
            const colleges = await College.find({ 
                slug: { $exists: true, $ne: null, $ne: '' }  // Only colleges with slugs
            })
            ...
            console.error("Error stack:", collegeError.stack);
        }
        ...
        // Send response
        res.send(xml);
        console.log("✅ Valid XML sitemap generated successfully");
